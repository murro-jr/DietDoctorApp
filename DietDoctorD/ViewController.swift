...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, GlobalTimerDelegate{
    ...
    var submarineSound:Sound = Sound(name:"Submarine",type:"aiff")
    var glassSound:Sound = Sound(name:"Glass",type:"aiff")
...
//        self.resetCoreData()
        ...
        currentWeightField.keyboardType = .numberPad
        ...
        weightGoalField.keyboardType = .numberPad
        ...
        do{
            ...
            if(result.count<=0){
                perform(#selector(presentInitiatingAlert), with: nil, afterDelay: 0)
            }else{
...
//                let timer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(updateDailyTime), userInfo: nil, repeats: true)
    ...
    func didUpdateGlobalTimer(_sender: GlobalTimer, message: String) {
        let currentDate: Date = dateFormatter.date(from: message as! String)!
        ...
        if(abs(now) > 86400){
            self.updateUserData()
            GlobalTimer.sharedTimer.setStartingTime(dateTime: currentDatetime)
        }
        print(now)
    }
    ...
    func updateUserData(){
        ...
        do{
            ...
            if(choice.compare("gain").rawValue == 0 && myCurrentWeight >= Double(newWeightGoal)!){
                ...
                message = "You've achieved your goal of " + newWeightGoal + " kg and lost " + String.localizedStringWithFormat("%.3f", abs(weightGain)) + " kg"
