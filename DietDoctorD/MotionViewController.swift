...
class MotionViewController: UIViewController {
    ...
    @IBAction func kilometerSelected(_ sender: Any) {
        ...
        if(kilometer.text!.count > 0){
            ...
            case 0:
                minutesResult = Double(kilometer.text?.replacingOccurrences(of: ",", with: "") as! String)!/0.060
                minute.text = NSString.localizedStringWithFormat("%.2f",minutesResult) as! String
                break
            case 1:
                minutesResult = Double(kilometer.text?.replacingOccurrences(of: ",", with: "") as! String)!*60/9
                minute.text = NSString.localizedStringWithFormat("%.2f",minutesResult) as! String
            default: break
            }
        }else{
    ...
    @IBAction func minuteSelected(_ sender: Any) {
        ...
        if(minute.text!.count > 0){
            switch motionType.selectedSegmentIndex{
            case 0:
                kmResult = Double(minute.text?.replacingOccurrences(of: ",", with: "") as! String)!*0.060
                kilometer.text = NSString.localizedStringWithFormat("%.2f",kmResult) as! String
                break
            case 1:
                kmResult = Double(minute.text?.replacingOccurrences(of: ",", with: "") as! String)!*9/60
                kilometer.text = NSString.localizedStringWithFormat("%.2f",kmResult) as! String
            default: break
            }
        }else{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        newMotion.setValue(kilometer.text?.replacingOccurrences(of: ",", with: ""), forKey: "kilometer")
        newMotion.setValue(minute.text?.replacingOccurrences(of: ",", with: ""), forKey: "minute")
